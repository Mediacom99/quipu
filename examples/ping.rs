...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut swarm =  p2::SwarmBuilder::with_new_identity()
        ...
        .with_swarm_config(|cfg| cfg.with_idle_connection_timeout(std::time::Duration::from_secs(u64::MAX))) // Allows us to observe pings indefinitely.
        .build();
    ...
    loop {
        ...
        match swarm.select_next_some().await {
            p2::swarm::SwarmEvent::NewListenAddr { address, .. } => println!("Listening on {address:?}"),
